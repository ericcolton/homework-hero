...
"""
Generate PDF worksheets from a JSON file. Supports either:
- A flat list of entries (one 3-page set total), or
- A structured object with top-level metadata and multiple "sections", each with its own entries (one 3-page set per section).
...
def _resolve_combined_out_path(combined_arg, base_output, json_path):
    """
    Decide a combined-PDF output path.
    Priority: explicit combined_arg > if base_output is a .pdf, append '-combined.pdf' > if base_output is a dir, 'combined.pdf' in it > default next to json.
    """
    if combined_arg:
        if combined_arg.lower().endswith(".pdf"):
            os.makedirs(os.path.dirname(combined_arg) or ".", exist_ok=True)
            return combined_arg
        # treat as directory
        os.makedirs(combined_arg, exist_ok=True)
        return os.path.join(combined_arg, "combined.pdf")
    if base_output:
        if os.path.isdir(base_output):
            return os.path.join(base_output, "combined.pdf")
        root, ext = os.path.splitext(base_output)
        if ext.lower() == ".pdf":
            return f"{root}-combined.pdf"
        # treat as directory path
        os.makedirs(base_output, exist_ok=True)
        return os.path.join(base_output, "combined.pdf")
    base = os.path.splitext(os.path.basename(json_path))[0]
    return os.path.join(os.path.dirname(json_path), f"{base}_combined.pdf")
def _derive_section_out_path(base_out_path, json_path, section_id):
    """
    Decide an output PDF path for a section. If base_out_path is provided and ends with .pdf,
    append "-section-<id>" before the extension. If it's a directory, put a
    "section-<id>.pdf" inside. If not provided, default next to json_path.
    """
    if base_out_path:
        if os.path.isdir(base_out_path):
            return os.path.join(base_out_path, f"section-{section_id}.pdf")
        root, ext = os.path.splitext(base_out_path)
        if ext.lower() == ".pdf":
            return f"{root}-section-{section_id}.pdf"
        # treat as directory path that may not yet exist
        os.makedirs(base_out_path, exist_ok=True)
        return os.path.join(base_out_path, f"section-{section_id}.pdf")
    # default next to json file
    base = os.path.splitext(os.path.basename(json_path))[0]
    return os.path.join(os.path.dirname(json_path), f"{base}_section_{section_id}.pdf")
...
def main():
    ...
    parser.add_argument("--combined-output", default=None, help="Create a single combined PDF of all sections. Supply a file path or directory.")
